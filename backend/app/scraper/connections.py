...
async def main(user_id=None):
    async with async_playwright() as p:
        ...
        #fetching the linkedin url of people from people
            approved=db.query(outreach,People).join(People).filter(outreach.human_approved=="Approved",outreach.status == "pending",outreach.user_id==user_id).all()
            for outreach_record,people in approved:
                ...
                await human_delay()
                if "premium" in page.url or "feed" in page.url:
                    await page.go_back()
                    await human_delay()
