...
def matching_score(state:AgentState):
    ...
    try:
        ...
        for job_id,jd in fetched_jd:
            ...
            job_record=db.query(Job).filter(Job.job_id==job_id,Job.user_id==user_id).first()
            ...
            db.commit()
        matched_score=db.query(Job).filter(Job.matching_score>=60,Job.user_id==user_id).all()  #storing only those which have score >60
    finally:    
        db.close()    
